 ...
 */
export async function createShortenedUrl(input: CreateShortenedUrlInput): Promise<ShortenedUrl> {
  const short_code = input.custom_code || generateShortCode();

  // Check if the short code already exists
  if (!input.custom_code) {
    const existingUrl = await db(TABLE_NAME)
      .where({ short_code })
      .first();
 ...
 */
export async function getShortenedUrlByCode(short_code: string): Promise<ShortenedUrl | null> {
 ...
 */
export async function getShortenedUrlsByUser(user_id: string): Promise<ShortenedUrl[]> {
  return db(TABLE_NAME)
    .where({ user_id })
    .orderBy("createdAt", "desc");
}
